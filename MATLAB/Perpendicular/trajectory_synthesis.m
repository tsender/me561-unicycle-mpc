...
theta = 0:0.01:2*pi;
hold on
plot((0.75*cos(theta)+8),(0.75*sin(theta)+5))
plot((0.75*cos(theta)+4),(0.75*sin(theta)+5))
plot((0.75*cos(theta)+4),(0.75*sin(theta)+3))
hold on
plot(0,0,'x');
...
function [g,h,dg,dh]=nonlcon(z)
    ...
    for i=1:nsteps
        ...
        g(3*i-2) = (1.25^2)-((z(3*i-2)-8)^2+(z(3*i-1)-5)^2) ;
        g(3*i-1) = (1.25^2)-((z(3*i-2)-4)^2+(z(3*i-1)-3)^2) ;
        g(3*i) = (1.25^2)-((z(3*i-2)-4)^2+(z(3*i-1)-5)^2) ;
        dg(3*i-2,3*i-2:3*i-1) = -2*[z(3*i-2)-8, z(3*i-1)-5] ;
        dg(3*i-1,3*i-2:3*i-1) = -2*[z(3*i-2)-4, z(3*i-1)-3] ;
